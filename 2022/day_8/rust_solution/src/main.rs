...
impl Matrix {
    ...
    fn get_column(&self, col: usize) -> Vec<u32> {
        self.matrix.iter().map(|row| *row.get(col).unwrap()).collect()
    }
    ...
    fn visible_to_right(&self, vector: &[u32], idx: usize) -> bool {
        vector[idx + 1..vector.len()].iter().all(|n| n < &vector[idx])
    }

    fn score_to_right(&self, vector: &[u32], idx: usize) -> usize {
        match vector[idx + 1..vector.len()].iter().position(|n| n >= &vector[idx]) {
    ...
    pub fn score(&self, row: usize, col: usize) -> usize {
        ...
        let row_score = self.score_to_left(&vector, col) 
        * self.score_to_right(&vector, col);

        // score from column
        let vector =  self.get_column(col);
        let col_score = self.score_to_left(&vector, row) 
        * self.score_to_right(&vector, row);
